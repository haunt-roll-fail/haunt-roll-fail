...
trait Tracker[T] {
    def get[S <: T](location : Location[T]) : List[S]
}
...
class IdentityTracker[T] extends Tracker[T] {
    ...
    private var entities : List[T] = Nil
    private var l2e : Map[Location[T], List[T]] = Map()
    private var e2l : Map[T, Location[T]] = Map()
    
    def another[S <: T](key : Any, content : List[S] = Nil, rule : S => Boolean = (e : S) => true) : AnonymousLocation[S, T] = {
        val l = new AnonymousLocation[S, T](key, this)
        register(l, content, rule)
        l
    }

    def register[S <: T](location : Location[T], content : List[S] = Nil, rule : S => Boolean = (e : S) => true) {
    ...
    def find(e : T) = e2l(e)
    def get[S <: T](location : Location[T]) : List[S] = l2e(location).map(_.asInstanceOf[S])
}
...
class ValueTracker[T] extends Tracker[T] {
    ...
    def anotherT(content : List[T] = Nil, rule : T => Boolean = (e : T) => true) : AnonymousLocation[T, T] = {
