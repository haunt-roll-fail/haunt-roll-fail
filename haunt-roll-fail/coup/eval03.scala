...
class GameEvaluation03(val game : Game, val self : Faction) extends GEvaluation {
    def eval(a : Action) : $[Evaluation] = {
        ...
        def death = hand == 1 && factions.but(self).exists(f => players(f).money >= 7)
        ...
        a.unwrap match {
            ...
            case StealFromAction(_, f) =>
                ...
                death && duel && players(f).money <= 8 |=> 1000000 -> "d&d"
