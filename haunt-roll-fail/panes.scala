...
class SplitHor(a : Pane, b : Pane) extends Pane {
    ...
    def dim(x : Int, y : Int, w : Int, h : Int) {
        ...
        val bi000 = cached.getOrElseUpdate(w * 10000 + h, { warn("split-hor not found"); 0 })
...
class SplitHorEven(panes : $[Pane]) extends Pane {
    def dim(x : Int, y : Int, w : Int, h : Int) {
        var n = x
...
object SplitHorEven {
    def apply(panes : $[Pane]) = new SplitHorEven(panes)
}

class SplitVerEven(panes : $[Pane]) extends Pane {
    def dim(x : Int, y : Int, w : Int, h : Int) {
        var n = y
...
object SplitVerEven {
    def apply(panes : $[Pane]) = new SplitVerEven(panes)
}
...
object SplitEven {
    def apply(panes : $[Pane]) = SplitEither(SplitVerEven(panes), SplitHorEven(panes))
}
...
class SplitZ(panes : $[Pane]) extends Pane {
    var cached = mutable.Map[Int, $[Pane]]()
...
object SplitZ {
    val cached = mutable.Map[$[Pane], SplitZ]()
