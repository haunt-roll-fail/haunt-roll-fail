...
class BotXX(f : Faction) extends EvalBot {
    def eval(game : Game, actions : List[UserAction]) : List[ActionEval] = {
        val ev = 
            if (f == Amalthea || true)
                new GameEvaluation(game, f)
            else
                new GameEvaluationOld(game, f)
...
class GameEvaluation(val game : Game, val self : Faction) extends GEvaluation {
    def eval(a : Action) : List[Evaluation] = {
        ...
        a.unwrap match {
            ...
            case KeepCardsAction(_, ll, _) =>
                ...
                l.has(Captain) |=> -800 -> "discard captain"
                l.has(Assassin) |=> -700 -> "discard assassin"
...
class GameEvaluationOld(val game : Game, val self : Faction) extends GEvaluation {
    def eval(a : Action) : List[Evaluation] = {
        var result : List[Evaluation] = Nil
