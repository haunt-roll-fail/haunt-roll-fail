...
case class IncomeAction(self : Faction) extends BaseAction(None)("Income")

case class ForeignAidAction(self : Faction) extends BaseAction(None)("Foreign Aid")

case class TaxAction(self : Faction) extends BaseAction(None)("Tax".styled(Duke))
case class TaxMoneyAction(self : Faction) extends ForcedAction

case class StealAction(self : Faction) extends BaseAction(None)("Steal".styled(Captain)) with Soft
...
case class AssassinateAction(self : Faction) extends BaseAction(None)("Assassinate".styled(Assassin)) with Soft
...
case class ExchangeAction(self : Faction) extends BaseAction(None)("Exchange".styled(Ambassador))
...
case class CoupAction(self : Faction) extends BaseAction(None)("Coup") with Soft
...
case class LoseInfluenceAction(f : Faction, e : Elem, then : ForcedAction) extends ForcedAction with Soft
...
case class BlockAction(f : Faction, self : Faction, r : Role, then : ForcedAction, fail : ForcedAction) extends BaseAction("Block with")(r) with Retry 
case class PassBlockAction(f : Faction, self : Faction, r : Role, then : ForcedAction, fail : ForcedAction) extends BaseAction("Block with")("Pass") with Retry
...
case class ViewCardInfoAction(self : Faction, card : Card) extends BaseInfo(self)(card.img) with Info with ViewCard
case class ViewTokenAction(self : Faction) extends BaseInfo(self)(Token.img) with Info with ViewToken
case class ViewHiddenTokenAction(self : Faction) extends BaseInfo(self)(HiddenToken.img) with Info with ViewToken
    ...
    def performZ(a : Action) : Continue = {
        ...
        action match {
            ...
            case TurnStartAction =>
                ...
                if (factions.num <= 1)
                    GameOverAction(factions.single.get, factions./~(players(_).hand))
                else {
            ...
            case BlockWithRoleAction(f, r, then, fail) =>
                if (blockers.none)
                    ...
                    MultiAsk(blockers./(b => Ask(b, BlockAction(b, f, r, then, fail) :: PassBlockAction(b, f, r, then, fail))))
            ...
            case KillBlockAction(f, v) =>
                ...
                Ask(v, v.hand.any.??(KillBlockWithAction(f, v, Contessa) :: Nil).refuse(KillKillAction(f, v)))
            ...
            case KillKillAction(f, v) =>
                if (v.hand.any) {
                    ...
                    LoseInfluenceAction(v, f.elem ~ " assassinated " ~ v.elem, TurnEndAction)
                }
                else
                    TurnEndAction
            ...
            case CoupWhomAction(f, v) =>
                ...
                log(f, "launched coup against", v)

                LoseInfluenceAction(v, f.elem ~ " launched coup against " ~ v.elem, TurnEndAction)
            ...
            case ChallengeInfluenceAction(f, r, then, fail) =>
                if (challengers.none)
                    ...
                    MultiAsk(challengers./(c => Ask(c, ChallengeAction(c, f, r, then, fail) :: PassAction(c, f, r, then, fail))))
            ...
            case ChallengeAction(e, f, r, then, fail) =>
                ...
                YYSelectObjectsAction(f, f.hand)
                    .withGroup(e.elem ~ " challenged " ~ r.elem)
            ...
            case RevealInfluenceAction(f, r, c, e, then, fail) =>
                if (c.role == r)
                    RevealRoleAction(f, c, r, FailAction(e, LoseInfluenceAction(e, e.elem ~ " failed " ~ "Challenge".hl, ReplaceInfluenceAction(f, then))))
                else
            ...
            case GameOverAction(f, l) =>
                ...
                log(f, "won with", l./(_.elem).join(" and "))
                
                GameOver($(f), "Game Over", $(
                    Ask(null, $(f)./~(f => f.hand./(ViewCardInfoAction(f, _)) :+ GameOverWonAction(null, f)))
                ))
