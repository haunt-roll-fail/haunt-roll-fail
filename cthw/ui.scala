...
class UI(val uir : ElementAttachmentPoint, val game : Game, val resources : Resources) extends GameUI with UI.GreyUI {
    ...
    def factionStatus(f : Faction, container : Container) {
        ...
        
        {
            ...
            val money = Empty // (Div(Image("token", styles.token), styles.viewcard).repeat(p.money)).some.|($(Div(Image("token", styles.token), styles.viewcard, xstyles.hidden)))
            val cards = Empty // p.hand./(c => Div(Image("hidden", styles.card), styles.viewcard)) ++ (p.reveal ++ p.lost.reverse)./(c => Div(c.img, styles.viewcard))
            // val c = claims.get(f).|(Nil)
            val claim = Empty // c.join((c.num > 3).?("|").|(" | "))
    ...
    ff => ff ++ true.? { 
        val ss = ff//.%(f => 1.to(game.setup.num)./("status-" + _).contains(f.name))
