...
class Game(val players : List[Player], val setup : List[Faction], val logging : Boolean, val options : List[GameOption]) extends BaseGame with ContinueGame with LoggedGame { game =>
    ...
    setup.foreach(f => pstates += f -> new PlayerState() { def faction = f; def game = game })

    def info(waiting : List[Faction], self : Option[Faction], actions : List[UserAction]) : List[UserAction with Info] = Nil
    ...
    def performZ(action : Action) : Continue = {
        ...
        if (cc.any) {
            ...
            case StartAction(v) =>
                ...
                log("Cthulhu Wars".hlb, "HRF".hl, hrf.Info.version.hlb)
