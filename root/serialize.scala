...
object Serialize extends Serializer {
    ...
    def es[_: P] = P( "$" ~ ("0" | "1" | "2" | "3").! ).map(_.toInt).map(EElderSign)
    
    def card[_: P] = P( "#" ~ number ).map(o => ECard(o.value))
    
    def quest[_: P] = P( "&" ~ number ).map(o => EQuest(o.value))
    
    def offer[_: P] = P( symbol ~ "->" ~ number ).map(o => EOffer(o._1.value, o._2.value))

    def itemref[_: P] = P( ("%%%%" | "%%%" | "%%" | "%").! ~ symbol ).map(o => EItemRef(o._1, o._2.value))

    def unitrefp[_: P] = P( symbol ~ "/" ~ action ~ "/" ~ number ).map(o => EUnitRefP(o._1.value, o._2, o._3.value))
    
    def unitref[_: P] = P( symbol ~ "/" ~ symbol ~ "/" ~ number ).map(o => EUnitRef(o._1.value, o._2.value, o._3.value))
      
    override def expr[_ : P] : P[Expr] = P( space ~ ( unitrefp | unitref | itemref | offer | es | card | quest | base ) ~ space )
    
    var version : String = null

    override def parseExpr(e : Expr) : Any = e match {
        case ESymbol("X") => AnyFund
        ...
        case ESymbol(s) => parseFaction(s).orElse(parseRegion(s)).orElse(parseDecree(s)).orElse(parseSymbol(s)).|!("unknow symbol " + e)
        case ECard(v) => Deck.full(v)
        case EQuest(v) => Quest.all(v)
        case EUnitRef(a, b, c) => Figure(parseRootFaction(a).get, parseSymbol(b).get.asInstanceOf[Piece], c)
        case EUnitRefP(a, b, c) => Figure(parseRootFaction(a).get, parseExpr(b).asInstanceOf[Piece], c)
        ...
        case _ => 
            version match {
                ...
                case "beta" =>
                    super.parseExpr(e)
                case null => 
                    super.parseExpr(e) match {
                        case s : String => s
                        case StartAction(v) => version = v; parseExpr(e)
                        case _ => throw new Error("unknown version " + version)
                    }
            }
