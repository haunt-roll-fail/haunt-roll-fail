...
package object html {
    ...
    case class Resources(assets : Map[String, dom.html.Image], names : () => Map[Any, String]) {
        def hasImage(key : String) = assets.contains(key.toLowerCase)
        def getImage(key : String) : dom.html.Image = assets.get(key.toLowerCase).|(throw new Error("asset " + key + " not found"))
        def getName(o : Any) : Option[String] = names().get(o)
        def getStyles(l : Seq[Style]) : List[String] = StyleRegister.add(l.toList)
    }

    class Container(val block : HtmlBlock, val attach : AttachmentPoint) {
        def node = block.nodes.single.get
        def hide() = block.hide()
        def show() = block.show()
    ...
    class HtmlBlock(val parent : dom.html.Element, val nodes : List[dom.html.Element]) {
        def hide() = nodes.foreach(_.style.display = "none")
        def show() = nodes.foreach(_.style.display = "")
        def invis() = nodes.foreach(_.style.visibility = "hidden")
        def vis() = nodes.foreach(_.style.visibility = "")
    ...
    class ElementAttachmentPoint(val parent : dom.html.Element) extends AttachmentPoint {
        ...
        def appendMultiContainer(elem : Elem, resources : Resources) : (List[dom.Node], Map[String, AttachmentPoint]) = {
            var cm = Map[String, AttachmentPoint]()
            def onContent(s : String, c : AttachmentPoint) = cm += s -> c
    ...
    def htmlString(elem : Elem, r : Resources) : String = elem match {
        ...
        case Image(image, styles) => "<img class='" + styles.id + "' src='" + r.getImage(image).src + "' />"
        case Concat(a, b) => htmlString(a, r) + "" + htmlString(b, r)
        case ElemList(l, j) => l./(e => htmlString(e, r)).join(htmlString(j, r))
        case Break => "<br/>"
    }

    def materialize(elem : Elem, resources : Resources, click : Any => Unit, over : Any => Unit, out : Any => Unit, params : List[Any], att : (String, AttachmentPoint) => Unit) : List[dom.html.Element] =
        materialize(elem : Elem, resources : Resources, click : Any => Unit, over : Any => Unit, out : Any => Unit, params : List[Any], att : (String, AttachmentPoint) => Unit, true)./(_.asInstanceOf[dom.html.Element])

    def materialize(elem : Elem, resources : Resources, click : Any => Unit, over : Any => Unit, out : Any => Unit, params : List[Any], att : (String, AttachmentPoint) => Unit, top : Boolean) : List[dom.Node] = elem match {
        case Empty => Nil
        ...
        case Image(s, styles) =>
            ...
            r.dataset("source") = s
            resources.getStyles(styles).foreach(r.classList.add)
            r.src = resources.getImage(s).src
            $(r)
        ...
        case Input(v, onChange, size, max, istyles, dstyles) =>
            ...
            materialize(e, resources, click, over, out,params, att, top).single match {
                case Some(r : dom.html.Element) =>
                    ...
                    r.onclick = params match {
                        case List(o) => (e) => { e.stopPropagation(); click(o) }
    ...
    object StyleRegister {
        private var existing : List[String] = Nil
        
        def add(styles : List[Style]) : List[String] = {
            val ss = styles.%(_.rules.any)
            if (ss.id.any && existing.has(ss.id).not) {
                var names = Set[String]()
                var rules = List[SimpleCSSRule]()
            ...
            (ss.id.some ++ styles.%(_.rules.none)./(_.name)).toList//.join(" ")
        }
    }
